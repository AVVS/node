...
const emptyArray = [];
const kNeverIndexFlag = StringFromCharCode(
  NGHTTP2_NV_FLAG_NO_INDEX |
  NGHTTP2_NV_FLAG_NO_COPY_NAME |
  NGHTTP2_NV_FLAG_NO_COPY_VALUE);
const kNoHeaderFlags = StringFromCharCode(
  NGHTTP2_NV_FLAG_NONE |
  NGHTTP2_NV_FLAG_NO_COPY_NAME |
  NGHTTP2_NV_FLAG_NO_COPY_VALUE);
...
function mapToHeaders(map,
  ...
  for (i = 0; i < keys.length; ++i) {
    ...
    if (isArray) {
      for (j = 0; j < value.length; ++j) {
        const val = typeof value[j] === 'string' ? value[j] : String(value[j]);
  ...
  const responseHeaders = pseudoHeaders + headers;
  responseHeaders | 0;

...
const {
  ...
  socket: kSocket,
  proxySocket: kProxySocket,
  request: kRequest,
} = binding.symbols;

const { Http2JSHeaders } = binding;
=======
...
const emptyArray = [];
const kNeverIndexFlag =
  NGHTTP2_NV_FLAG_NO_INDEX |
  NGHTTP2_NV_FLAG_NO_COPY_NAME |
  NGHTTP2_NV_FLAG_NO_COPY_VALUE;
const kNoHeaderFlags =
  NGHTTP2_NV_FLAG_NONE |
  NGHTTP2_NV_FLAG_NO_COPY_NAME |
  NGHTTP2_NV_FLAG_NO_COPY_VALUE;
...
function mapToHeaders(map,
  ...
  const headers = new Http2JSHeaders();
  ...
  for (i = 0; i < keys.length; ++i) {
    ...
    if (isArray) {
      arrLen = value.length;
      if (arrLen === 1) {
          value = typeof value[0] === 'string' ? value[0] : String(value[0]);
          isArray = false;
      } else if (arrLen > 1) {
    ...
    if (key[0] === ':') {
      ...
      if (err !== undefined)
        throw err;
      headers.addHeader(key, value, flags);
      continue;
    }
    if (key.includes(' ')) {
    ...
    if (isArray) {
      ...
      for (j = 0; j < value.length; ++j) {
        val = typeof value[j] === 'string' ? value[j] : String(value[j]);
        headers.addHeader(key, val, flags);
      }
      continue;
    }

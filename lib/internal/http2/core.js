...
const { constants, symbols, nameForErrorCode } = binding;
...
function requestOnConnect(http2Headers, options) {
  const session = this[kSession];
  ...
  const ret = session[kHandle].request(http2Headers,
                                       streamOptions,
                                       options.parent | 0,
                                       options.weight | 0,
                                       !!options.exclusive);
...
class ClientHttp2Session extends Http2Session {
  ...
  request(headers, options) {
    ...
    const http2Headers = mapToHeaders(headers);
    ...
    const onConnect = requestOnConnect.bind(stream, http2Headers, options);
    if (this.connecting) {
...
function finishSendTrailers(stream, http2headers) {
  ...
  stream[kState].flags &= ~STREAM_FLAGS_HAS_TRAILERS;
  const ret = stream[kHandle].trailers(http2headers);
...
class Http2Stream extends Duplex {
  ...
  sendTrailers(headers) {
    ...
    const http2headers = mapToHeaders(headers, assertValidPseudoHeaderTrailer);
    this[kSentTrailers] = headers;

    // do it in here as errors are thrown from c++
    // finishSendTrailers(this, headers);

    // Send the trailers in setImmediate so we don't do it on nghttp2 stack.
    setImmediate(finishSendTrailers, this, http2headers);
  }
...
function processRespondWithFD(self, fd, headers, offset = 0, length = -1,
  ...
  let ret;
  try {
    ret = self[kHandle].respond(headers, streamOptions);
  } catch (err) {
    self.destroy(err);
    return;
  }
...
class ServerHttp2Stream extends Http2Stream {
  ...
  pushStream(headers, options, callback) {
    ...
    const ret = this[kHandle].pushPromise(http2headers, streamOptions);
    let err;
    if (typeof ret === 'number') {
      switch (ret) {
        case NGHTTP2_ERR_STREAM_ID_NOT_AVAILABLE:
  ...
  respond(headers, options) {
    ...
    const processedHeaders = processHeaders(headers, options);
    ...
    const statusCode = processedHeaders[HTTP2_HEADER_STATUS] | 0;
    if (!!options.endStream ||
    ...
    const http2headers = mapToHeaders(processedHeaders, assertValidPseudoHeaderResponse);

    // can only be set after validation
    state.flags |= STREAM_FLAGS_HEADERS_SENT;
    this[kSentHeaders] = processedHeaders;

    const ret = this[kHandle].respond(http2headers, streamOptions);
  ...
  additionalHeaders(headers) {
    ...
    const ret = this[kHandle].info(headers);
    if (ret < 0)
      this.destroy(new NghttpError(ret));
  }
}

...
function requestOnConnect(headers, options) {
  ...
  let ret;
  try {
    ret = session[kHandle].request(headers,
                                   streamOptions,
                                   options.parent | 0,
                                   options.weight | 0,
                                   !!options.exclusive);
...
function finishSendTrailers(stream, headersList, headerCount) {
  ...
  let ret;
  try {
    ret = stream[kHandle].trailers(headersList, headerCount);
  } catch (err) {
...
class Http2Stream extends Duplex {
  ...
  sendTrailers(headers) {
    ...
    setImmediate(finishSendTrailers, this, headersList[0], headersList[1]);
  }
...
class ServerHttp2Stream extends Http2Stream {
  ...
  respond(headers, options) {
    ...
    const [headerList, headerCount] = mapToHeaders(processedHeaders, assertValidPseudoHeaderResponse);

    // can only be set after validation
    state.flags |= STREAM_FLAGS_HEADERS_SENT;
    this[kSentHeaders] = processedHeaders;

    // console.log('prepared headers', headerList[1], headerList[0], processedHeaders);
    const ret = this[kHandle].respond(headerList, headerCount, streamOptions);
